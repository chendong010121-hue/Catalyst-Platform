...
```python
# RuntimeDomain 把 session 持久化 namespace 与 execution safety control namespace 绑定
domain = RuntimeDomain(state_store=state_store)   # 或显式 execution_control_plane=...
runtime_a = Runtime(reasoner=reasoner, capabilities=capabilities, policy=policy, domain=domain)
...
- **shared execution control plane（RuntimeDomain identity）**：`RuntimeDomain` 绑定 StateStore + `ExecutionControlPlane`（含 `ActiveExecutionRegistry` + `LateCompletionEvidenceRegistry`）。`Runtime` 只能由 domain 构造（无独立 store/control_plane），结构上杜绝"同一 store + 两套 control plane"形成 invisible safety island。下层 `DefaultCapabilityExecutor` 若 timeout enabled 且无 control plane → `RuntimeConfigurationError`。
