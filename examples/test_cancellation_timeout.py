...
def _runtime(capability, timeout_config=None, store=None, reasoner=None, control_plane=None):
    # 测试 helper：timeout-enabled 时若未显式给 control_plane，自动提供一个（单 Runtime 场景）。
    if (
        control_plane is None
        and timeout_config is not None
        and timeout_config.timeout_seconds is not None
    ):
        control_plane = ExecutionControlPlane()
    return Runtime(reasoner=reasoner or ActThenCompleteReasoner(), capabilities={"add": capability}, policy=AllowAllPolicy(), timeout_config=timeout_config, domain=RuntimeDomain(state_store=store or InMemoryStateStore(), execution_control_plane=control_plane))
