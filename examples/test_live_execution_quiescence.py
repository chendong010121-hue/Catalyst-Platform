...
def _make_live_uncertain(cap, control_plane, store, timeout=0.05, grace=0.1):
    rt = Runtime(ActThenCompleteReasoner(), {"add": cap}, AllowAllPolicy(), timeout_config=ExecutionTimeoutConfig(timeout_seconds=timeout, cancellation_grace_seconds=grace), domain=RuntimeDomain(state_store=store, execution_control_plane=control_plane))
...
def test_q1_q2_registry_retains_and_cancel_targets_live():
    ...
    # cleanup
    cap.release.set()
    _wait_registry_cleared(control_plane, sid)
...
def test_q8_spurious_execution_cancelled_unresolved():
    store = InMemoryStateStore()
    rt = Runtime(ActThenCompleteReasoner(), {"add": SpuriousCancelCapability()}, AllowAllPolicy(), domain=RuntimeDomain(state_store=store))
    snap = rt.create(Goal("x"))
    sid = snap.session_id
    try:
        rt.run(sid)
...
def test_q9_requested_cooperative_cancel_settles():
    ...
    store = InMemoryStateStore()
    rt = Runtime(ActThenCompleteReasoner(), {"add": cap}, AllowAllPolicy(), domain=RuntimeDomain(state_store=store))
    snap = rt.create(Goal("x"))
    sid = snap.session_id
...
def test_q10_task_timeout_classified_as_execution_error():
    store = InMemoryStateStore()
    rt = Runtime(ActThenCompleteReasoner(), {"add": TaskTimeoutCapability()}, AllowAllPolicy(), timeout_config=ExecutionTimeoutConfig(timeout_seconds=10.0, cancellation_grace_seconds=0.5), domain=RuntimeDomain(state_store=store, execution_control_plane=ExecutionControlPlane()))
    snap = rt.create(Goal("x"))
    sid = snap.session_id
    try:
        rt.run(sid)
