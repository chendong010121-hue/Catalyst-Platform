"""FN-11 Local Runner / Service Shell (PRIVATE / DEFERRED) + whole-Agent pipeline (E1 EXTENDED).
...
E1 extension: generalized local evidence-query dispatch (spec §9):
...
ids, NO per-benchmark conclusion strings (E1 anti-fixture rule, spec §4).
"""
...
# branches — spec §15 allows professional applicability rules).
_APPLICABILITY_TRIGGERS = ("防火间距", "停车位", "配建", "机动车停车")
...
def answer(
    ...
    # ---- QMODE-05: existing professional applicability (T-C01/02/03 preserved) ----
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="brea", description="BREA v0.2-candidate runner")
