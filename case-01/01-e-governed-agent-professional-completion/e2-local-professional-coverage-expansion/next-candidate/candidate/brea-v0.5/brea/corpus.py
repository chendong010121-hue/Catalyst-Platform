...
_CASE_ROOT = Path(__file__).resolve().parents[6]  # case-01/ (next-candidate adds one level)
...
def load_corpora(case_root_path: Path | None = None) -> dict[str, Corpus]:
    ...
    for row in load_manifest_rows(manifest):
        ...
        corpora[Path(row["file"]).stem] = corpus  # also key by admitted manifest file stem
    return corpora
