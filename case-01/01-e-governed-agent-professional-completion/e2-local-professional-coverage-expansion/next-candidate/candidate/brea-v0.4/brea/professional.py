...
from .domain_data import fact_descriptors, route_by_name
...
GB_KEY = "GB55037-2022"
DBJ_KEY = "DBJ33T1021-2023"
GB = "GB 55037-2022"
DBJ = "DBJ33/T1021-2023"
...
def _closed(
    ...
    return build_result(
        ...
        professional_trace=trace,
    )


def _retrieve_clause(route: dict, corpora: dict) -> tuple[list, str, dict] | None:
    ...
    if found is None:
        return None
    source_id, title, version = _source_meta(route["standard_id"])
    locator = (
...
def _retrieve_table(route: dict, corpora: dict, facts: dict, chain: dict) -> tuple[list, str, dict] | None:
    corpus = corpora.get(route["standard_id"])
    ...
    item_level = make_evidence_item(
        ...
        f"第5.0.1条+表5.0.1（OCR 副本 [page {page_of(corpus, start - 1)}]，本地副本行 {start}-{end}）",
...
def _conclusion(route: dict, applicability: dict, numeric: dict | None, table: dict) -> str:
    ...
    if route["kind"] == "table_rule":
        return (
            f"按 DBJ33/T1021-2023 表5.0.1（城市类别 → 指标级别 {table['selected_column']}）"
            f"与源表行，机动车配建指标不应小于 {table['selected_value']:g} 车位/100m² 建筑面积。"
        )
...
def answer_professional(
    ...
    chain = applicability_for_question(question, facts, dbj)
    route_name = chain.get("route")
    route = route_by_name(route_name) if route_name else None
    if route is not None:
        ...
        if missing:
            ...
            return _closed(request_id, status, conclusion, uncertainty, attribution, corpus_sha,
                           _trace(applicability={"owner": "SEAM-02", "state": "unresolved", "missing": labels}),
                           out_dir, chain.get("standard_id"))
    if route is None or not chain.get("standard_id"):
        status, conclusion, uncertainty = decide([], chain, bound_ok=False)
        return _closed(request_id, status, conclusion, uncertainty, attribution, corpus_sha,
                       _trace(applicability={"owner": "SEAM-02", "state": "unresolved"}), out_dir)

    retrieved = _retrieve_table(route, corpora, facts, chain) if route["kind"] == "table_rule" else _retrieve_clause(route, corpora)
    ...
    return build_result(
        ...
        engine="brea-deterministic-v0.4",
    )
