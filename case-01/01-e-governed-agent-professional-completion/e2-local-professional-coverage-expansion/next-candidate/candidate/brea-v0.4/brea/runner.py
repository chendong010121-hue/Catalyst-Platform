"""BREA v0.4 deterministic product runner.
...
from .applicability import LEVEL_COLUMN
...
def _qm01_clause_lookup(request_id, standard_id, clause_id, corpora, attribution, out_dir):
    corpus = corpora[standard_id]
    source_id, title, version = _source_meta(standard_id)
...
def _qm03_topic_search(request_id, question, standard_id, corpora, attribution, out_dir):
    corpus = corpora[standard_id]
    source_id, title, version = _source_meta(standard_id)
...
def _qm04_table_region(request_id, question, standard_id, table_number, corpora, facts, attribution, out_dir):
    corpus = corpora[standard_id]
    source_id, title, version = _source_meta(standard_id)
...
def answer(
    ...
    if mode == "QMODE-01":
        return _qm01_clause_lookup(request_id, classification["standard_id"], classification["clause_id"], corpora, attribution, out_dir)
    if mode == "QMODE-04":
        return _qm04_table_region(request_id, question, classification["standard_id"], classification["table_number"], corpora, facts, attribution, out_dir)
    if mode == "QMODE-03":
        return _qm03_topic_search(request_id, question, classification["standard_id"], corpora, attribution, out_dir)
    return answer_professional(
        request_id, question, facts, corpora, attribution,
        {key: value.expected_sha for key, value in corpora.items()}, out_dir,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="brea", description="BREA v0.4-candidate runner")
    ...
    print(json.dumps(_run_request(data, args.out).to_dict(), ensure_ascii=False, indent=2))
    return 0
