...
def match_route(question: str, facts: dict, dbj: Corpus | dict | None, knowledge: dict) -> dict | None:
    """Resolve one declarative professional route without family dispatch."""
    candidates = [route for route in routes(knowledge) if _intent_matches(question, route)]
    if not candidates:
        return None
    route = candidates[0]
