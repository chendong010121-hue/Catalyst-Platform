"""Explicit Case-local Knowledge Revision binding for BREA v0.8.
...
def _canonical_knowledge_sha(document: dict[str, Any]) -> str:
    ...
    Serialize the full Knowledge Revision EXCEPT sources[].local_reference with
    stable object-key ordering and compact, whitespace-free separators encoded
    as UTF-8. Non-standard JSON numeric constants are not supported and may not
    survive a strict parse, so allow_nan=False keeps them from leaking into the
    identity.
    """
