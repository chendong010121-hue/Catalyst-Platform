...
from .applicability import (
    applicability_for_question,
    canonical_level_label,
    level_column,
    resolve_semantic_applicability,
)
from .corpus import line_range, norm, page_of, table_region
from .coverage import extract_full_clause, parse_numbered_items
from .domain_data import fact_descriptors, standard_meta
from .evidence import (
    assert_verbatim,
    locate_clause,
    locate_numeric_banded_row,
    locate_table_row,
    make_evidence_item,
)
...
def _retrieve_clause(route: dict, corpora: dict, knowledge: dict) -> tuple[list, str, dict] | None:
    ...
    if route["kind"] == "conditional_rule":
        items = []
        for subitem in parse_numbered_items(found["text"]):
            sub_locator = f"{locator[:-1]};{route['locator']}({subitem['index']})）"
            item = make_evidence_item(
                source_id, title, version, sub_locator, evidence_type, subitem["text"], "supports"
            )
            assert_verbatim(corpus, item.evidence_content)
            items.append(item)
        if items:
            return items, found["text"], {"source_id": source_id, "locator": locator, "raw": found["text"]}
...
def _retrieve_numeric_banded_table(
    route: dict, corpora: dict, facts: dict, knowledge: dict
) -> tuple[list, str, dict] | None:
    corpus = corpora.get(route["standard_id"])
    selector = route.get("selector") or {}
    selector_fact = selector.get("fact")
    if corpus is None or selector_fact not in facts:
        return None
    caption = route["table_caption"]
    found = locate_numeric_banded_row(
        corpus, caption, facts[selector_fact], selector, route.get("outputs", [])
    )
    if found is None:
        return None
    source_id, title, version = _source_meta(knowledge, route["standard_id"])
    region_start, region_end = line_range(corpus, caption, len(found["region"].split("\n")))
    item_region = make_evidence_item(
        source_id,
        title,
        version,
        f"{caption}（OCR 副本 [page {page_of(corpus, region_start - 1)}]，本地副本行 {region_start}-{region_end}）",
        "table_value",
        found["region"],
        "supports",
    )
    ...
    assert_verbatim(corpus, item_region.evidence_content)
    assert_verbatim(corpus, item_row.evidence_content)
    return [item_region, item_row], found["region"] + "\n" + found["label"], {
        "source_id": source_id,
        "locator": item_row.locator,
        "raw": found["region"] + "\n" + found["label"],
        "row_label": found["label"],
        "selected_band": found["selected_band"],
        "selected_outputs": found["selected_outputs"],
...
def _retrieve_table(route: dict, corpora: dict, facts: dict, chain: dict, knowledge: dict) -> tuple[list, str, dict] | None:
    if route.get("selector", {}).get("fact"):
        return _retrieve_numeric_banded_table(route, corpora, facts, knowledge)
    corpus = corpora.get(route["standard_id"])
    level = chain.get("level")
    if corpus is None or level is None:
        return None
    caption = route["table_caption"]
    found = locate_table_row(
        corpus,
        caption,
        facts.get("building_category"),
        float(facts["floor_area_m2"]),
        5,
        tuple(route.get("scope_terms", {}).get("building_category", ())),
    )
    if found is None:
        return None
    column = level_column(corpus, level, route["level_resolution"]["caption"])
    ...
    row_content = "\n".join(found["raw_values"])
    row_evidence = table_region(corpus, caption) + "\n" + row_content
    ...
    item_row = make_evidence_item(
        source_id, title, version,
        f"{caption}（OCR 副本 [page {found['page']}]，本地副本行 {found['start']}-{found['end']}），行：{found['label']}",
        "table_value", row_evidence, "supports",
    )
    ...
    return [item_level, item_row], raw, {
        ...
        "selected_column": canonical_level_label(level),
        "source_level_label": level,
        "calculated_total": float(selected) * float(facts["floor_area_m2"]) / 100.0,
        "calculation": {
            "formula": f"{selected} * {facts['floor_area_m2']:g} / 100",
            "source_indicator": float(selected),
            "floor_area_m2": float(facts["floor_area_m2"]),
        },
    }


def _numeric_trace(view: dict, applicability: dict, facts: dict, table: dict | None = None) -> dict | None:
    if table and table.get("selected_value") is not None:
        return {
            "source_operand": {
                "kind": "source_table_cell",
                "selected_column": table.get("selected_column"),
                "row_label": table.get("row_label"),
            },
            "source_value": table["selected_value"],
            "formula": table.get("calculation", {}).get(
                "formula", "source table cell selected by source-derived level and row"
            ),
            "result": table.get("calculated_total", table["selected_value"]),
    ...
    if not operands:
        return None
    operand = operands[0]
    if operand.get("kind") == "source_banded_table_row":
        if not table or not table.get("selected_outputs"):
            return None
        selector_fact = operand.get("selector_fact")
        return {
            "source_operand": {
                "kind": "source_banded_table_row",
                "selector_fact": selector_fact,
                "selector_value": facts.get(selector_fact),
                "band": table.get("selected_band"),
                "row_label": table.get("row_label"),
            },
            "source_outputs": table["selected_outputs"],
            "formula": "source row outputs selected by declarative numeric band",
            "source_evidence_bound": True,
        }
...
def _conclusion(route: dict, applicability: dict, numeric: dict | None, table: dict) -> str:
    outcome = (applicability.get("rule") or {}).get("outcome") or {}
    if route.get("selector", {}).get("fact"):
        outputs = table.get("selected_outputs", {})
        rendered = "；".join(
            f"{output['label']} {outputs[output['name']]['raw']}"
            for output in route.get("outputs", [])
            if output["name"] in outputs
        )
        qualification = route.get("qualification", {})
        return (
            f"按 {table['source_id']} {route['table_caption']} 选中源表行：{table['row_label']}；"
            f"{rendered}；表注：{qualification.get('height_note', '')}；"
            f"{qualification.get('unsupported_claim', '')}。"
        )
    if route["kind"] == "table_rule":
        total = table.get("calculated_total")
        total_text = f"；按项目建筑面积折算，机动车位数量为 {total:g} 个" if total is not None else ""
        return (
            f"按 {table['source_id']} {route['level_resolution']['caption']}"
            f"（城市类别 → 指标级别 {table['selected_column']}）与源表行，"
            f"{route['conclusion_subject']}不应小于 {table['selected_value']:g} {route['conclusion_unit']}{total_text}。"
        )
...
def answer_professional(
    ...
    chain = applicability_for_question(question, facts, corpora, knowledge, regulation_context)
    ...
    if route is not None:
        ...
        unsupported_terms = (route.get("qualification") or {}).get("unsupported_request_terms", [])
        if any(term in question for term in unsupported_terms):
            return _closed(
                request_id,
                "no_reliable_evidence",
                "请求超出当前专业切片边界，不能据此宣称完整项目合规性。",
                Uncertainty(level="explicit", description="unsupported professional claim"),
                attribution,
                corpus_sha,
                _trace(applicability={"owner": "SEAM-02", "state": "unresolved", "reason": "unsupported claim"}, knowledge_info=knowledge_info),
                out_dir,
                chain.get("standard_id"),
                knowledge_info,
            )
    if route is None or not chain.get("standard_id"):
        ...
        if route is not None:
            ...
            conclusion = (
                "本地已接纳规范库中无适用依据，不能给出规范结论；"
                "请补充确认项目法域或提供已绑定的正式来源后再判断。"
            )
            uncertainty = Uncertainty(level="explicit", description="适用性或正式来源未解析")
        return _closed(request_id, status, conclusion, uncertainty, attribution, corpus_sha,
                       _trace(applicability={"owner": "SEAM-02", "state": "unresolved"}, knowledge_info=knowledge_info), out_dir,
                       knowledge_info=knowledge_info)

    retrieved = _retrieve_table(route, corpora, facts, chain, knowledge) if route["kind"] in {"table_rule", "numeric_banded_table"} else _retrieve_clause(route, corpora, knowledge)
    ...
    table = source if route["kind"] in {"table_rule", "numeric_banded_table"} else {}
    numeric = _numeric_trace(semantic_view, resolved, facts, table) if resolved.get("state") == "applicable" else None
    contract = {
    ...
    return build_result(
        ...
        engine="brea-deterministic-v0.8",
        knowledge_info=knowledge_info,
    )
