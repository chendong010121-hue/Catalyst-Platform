...
Deterministic local retrieval over the admitted corpora. NO LLM / RAG / Web /
...
_PROFESSIONAL_OUTCOME_INTENT = (
    "适用", "适用性", "专业结论", "结论", "判定", "判断", "计算", "最大", "最小",
    "上限", "下限", "应为", "是多少", "多少", "多大", "能否", "是否", "需要哪些", "缺失", "指标",
)


def is_professional_outcome_intent(question: str) -> bool:
    """Recognize an outcome-seeking request independent of evidence wording."""
    return any(marker in question for marker in _PROFESSIONAL_OUTCOME_INTENT)
...
def classify_query(question: str, regulation_context: dict | None, knowledge: dict) -> dict:
    ...
    if standard_id is not None and is_professional_outcome_intent(question):
        return {"mode": "QMODE-05", "standard_id": standard_id, "clause_id": None,
