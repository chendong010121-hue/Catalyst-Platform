...
def _scope(
    ...
    # Declarative fact scope is independent of the retrieved prose. This keeps
    # positive project applicability in SEAM-02 while allowing a table route to
    # bind its scope facts from the project context and its numeric outputs from
    # the source row.
    for rule in route.get("scope_fact_rules", []):
        fact = rule.get("fact")
        allowed_values = rule.get("allowed_values", [])
        if fact and allowed_values:
            positive.append({"fact": fact, "op": "in", "value": allowed_values})
...
def _modifier_rules(
    ...
    if not route.get("modifier_condition_facts"):
        return []
...
def _numeric(
    ...
    if selector.get("fact"):
        return {
            "operands": [{
                "kind": "source_banded_table_row",
                "selector_fact": selector["fact"],
            }],
...
def derive_semantic_view(
    ...
    selector = route.get("selector") or {}
    if selector.get("fact"):
        value = facts.get(selector["fact"])
        selected_band = next(
            (
                band for band in selector.get("bands", [])
                if isinstance(value, (int, float))
                and band.get("min") <= value <= band.get("max")
            ),
            None,
        )
        rules = [{
            "all": [{
                "fact": selector["fact"],
                "op": "between",
                "min": selected_band.get("min") if selected_band else None,
                "max": selected_band.get("max") if selected_band else None,
            }],
            "outcome": {"effect": "source_banded_table_row", "band": selected_band},
        }]
    ...
    return {
        ...
        "numeric": _numeric(raw, facts, route, rules, modifier_rules),
    }
