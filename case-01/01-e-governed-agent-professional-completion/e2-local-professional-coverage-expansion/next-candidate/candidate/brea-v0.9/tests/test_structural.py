...
def _binding() -> dict:
    return canonical_binding(KR_PATH)

import brea  # noqa: E402,F401  (import smoke: package importable)
from brea.corpus import Corpus, CorpusIntegrityError, load_corpora  # noqa: E402
...
def _run_fixture(name: str, ent: dict):
    ...
    return answer(
        ...
        request["regulation_context"], ent, knowledge_binding=_binding(),
    )
...
def test_st06_numeric_traceability() -> None:
    knowledge = __import__("json").loads(KR_PATH.read_text(encoding="utf-8"))
    _ = load_corpora(knowledge["sources"])  # corpus present + SHA verified
...
def test_st07_corpus_sha_fail_closed() -> None:
    knowledge = json.loads(KR_PATH.read_text(encoding="utf-8"))
    row = knowledge["sources"][0]
    try:
